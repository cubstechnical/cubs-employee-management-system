...
CREATE INDEX IF NOT EXISTS idx_notification_logs_urgency ON notification_logs(urgency);
CREATE INDEX IF NOT EXISTS idx_notification_logs_created_at ON notification_logs(created_at);
CREATE INDEX IF NOT EXISTS idx_notification_logs_manual_trigger ON notification_logs(manual_trigger);
...
CREATE POLICY "Allow authenticated users to read notification logs" ON notification_logs
    FOR SELECT USING (auth.role() = 'authenticated');

-- Allow service role to insert/update notification logs
CREATE POLICY "Allow service role to manage notification logs" ON notification_logs
...
-- Allow service role to manage email templates
CREATE POLICY "Allow service role to manage email templates" ON email_templates
    FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');
...
CREATE OR REPLACE FUNCTION get_expiring_visas_on_days(days INTEGER[])
RETURNS TABLE (
    employee_id TEXT,
    emp_id TEXT,
    name TEXT,
...
CREATE OR REPLACE FUNCTION get_expiring_visas_within_days(threshold_days INTEGER DEFAULT 90)
RETURNS TABLE (
    employee_id TEXT,
    emp_id TEXT,
    name TEXT,
